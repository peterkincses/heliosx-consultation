...
const Footer = () => {
    ...
    const numberOfGroups = new Set(footerLinks.map(link => link.group))?.size ?? 0;

    return (
        <footer className="border-t border-gray-200">
            <div className="flex gap-6 flex-wrap container mx-auto p-5 flex-col md:flex-row">
                {Array.from({ length: numberOfGroups })?.map((_, i: number) => (
                    <div key={`footer-column-${i}`} className="min-w-[200px]">
                        <ul className="flex flex-col gap-y-2">
                            {footerLinks?.filter(link => link.group === `column-${i + 1}`)?.map((link) => (
